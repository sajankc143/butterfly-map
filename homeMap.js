...
async function loadHomeObservations() {
   ...
   // REPLACE THE proxyServices ARRAY AND fetchWithFallbacks FUNCTION WITH THIS:
...
async function fetchWithFallbacks(url) {
    // Try direct fetch first (works if same domain or CORS enabled)
    try {
    ...
    for (let proxyIndex = 0; proxyIndex < proxyServices.length; proxyIndex++) {
        ...
        for (let retry = 0; retry < maxRetries; retry++) {
            try {
                ...
                console.log(`Trying proxy ${proxyIndex + 1} (${proxy.url}), attempt ${retry + 1}`);
                ...
                if (response.ok) {
                    ...
                    if (content && content.length > 1000) {
                        ...
                        console.log(`Content too short: ${content ? content.length : 0} characters`);
                ...
                if (retry < maxRetries - 1) {
                    const delay = 2000 + (retry * 1000); // Increased delays
